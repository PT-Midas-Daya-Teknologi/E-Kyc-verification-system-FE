/**
 * Eye Aspect Ratio (EAR) blink detection using MediaPipe Face Mesh landmarks.
 * Uses vertical/horizontal eye distances; blink = sharp dip then recovery.
 */
...
/** Average EAR for both eyes (stable across neutral expressions). */
...
/**
 * Stateful blink detector: detects one deliberate blink (closed phase then reopen).
 * Rejects flat/static signals by requiring a minimum drop below baseline.
 */
export function createBlinkDetectorState() {
  return {
    ...
    baselineReady: false,
    baselineEar: 0.28,
    phase: 'open', // 'open' | 'closing' | 'reopening'
    closedFrames: 0,
    satisfied: false,
    peakDrop: 0,
  };
...
const CLOSED_THRESHOLD_RATIO = 0.72; // ear drops below baseline * ratio
