...
function KYCVerificationPage() {
  ...
  const [cameraStatus, setCameraStatus] = useState('Click Start KYC to begin');
  ...
  const handleDetect = useCallback(
    (payload) => {
      ...
      if (payload.faceCount === 0) setCameraStatus('Face not detected');
      else if (payload.faceCount > 1) setCameraStatus('Multiple faces detected');
      else setCameraStatus('Face detected');
    },
    [processFrame]
  );

  useEffect(() => {
    if (!sessionStarted || !mediaReady || recorderStartedRef.current) return;
    let cancelled = false;
    let attempts = 0;
    const startRecording = () => {
      ...
      attempts += 1;
      if (!cancelled && attempts < 80) requestAnimationFrame(startRecording);
    };
    startRecording();
    return () => {
      cancelled = true;
  ...
  // When verification completes: stop camera, stop recorder, then navigate
  // once onstop has fired and the blob is fully assembled.
  useEffect(() => {
    if (!completed) return;
    stopCamera();
    ...
    recorder.stop((blob) => {
      ...
      navigate('/recording', {
        state: { previewUrl: url, fileName: 'kyc-verification.webm' },
      });
    });
  ...
  const onBack = () => {
    ...
    setMediaReady(false);
    setCameraStatus('Click Start KYC to begin');
  };
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-10">
      <div className="mx-auto max-w-xl">
        <header className="mb-6 text-center">
          <h1 className="text-3xl font-bold text-sky-700 sm:text-4xl">Active Liveness Verification</h1>
          <p className="mt-2 text-sm text-slate-600">
            Complete the instructions below to verify your identity.
          </p>
        </header>
          ...
          {!sessionStarted ? (
            <div className="text-center">
              <button
                type="button"
                onClick={onStartKyc}
                className="rounded-lg bg-sky-600 px-5 py-2.5 font-semibold text-white transition hover:bg-sky-700"
              >
                Start KYC
              </button>
            </div>
            ...
            <div>
              ...
              />
              <FaceDetection videoRef={webcamRef} enabled={detectorEnabled} onDetect={handleDetect} />
              ...
              <p className="mt-3 text-center text-sm text-slate-700">{statusText}</p>
              <p className="mt-1 text-center text-xs text-slate-500">{cameraStatus}</p>
              <VerificationButtons onRestart={onRestart} onBack={onBack} onRecording={onRecordingPreview} />
            </div>
          )}
        </motion.div>
      </div>
